...
Project generation writes responsibility-specific files under each source package's `gen`
subpackage. Every run rebuilds complete files and atomically replaces existing Goark-owned
targets. Goark adds `**/gen/` to `.gitignore`; these local artifacts are not committed and are
excluded from source layout limits. Ownership is identified by:

```go
// Code generated by goark; DO NOT EDIT.
```

Goark refuses to overwrite a same-named handwritten file. When `clean-stale = true`, only
obsolete files carrying this exact ownership header are removed. A dry run reports changes
without creating, replacing, or deleting files.
