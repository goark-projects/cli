...
`
	...
	if err == nil ||
		!strings.Contains(
			err.Error(),
			`annotation "service" has unsupported trailing content "trailing"`,
		) {
...
func TestGenerateAnnotations_whenCoreAnnotationArgumentInvalid_shouldReturnValidationError(
	t *testing.T,
) {
	cases := []struct {
		name          string
