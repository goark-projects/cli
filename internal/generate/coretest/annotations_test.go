...
func TestGenerateAnnotations_whenPackageHasGoarkAnnotations_shouldGenerateConfiguration(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenBeanParameterUnannotated_shouldInferFactoryDependency(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenOptionalAutowiredProvided_shouldIgnoreOnlyMissingBean(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
