...
func TestGenerateAnnotations_whenRequestMappingHasNoMethod_shouldGenerateDefaultMethodRoutes(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenRequestMappingHasMultipleMethods_shouldGenerateRoutePerMethod(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
