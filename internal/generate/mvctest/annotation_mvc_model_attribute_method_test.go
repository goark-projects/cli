...
func TestGenerateAnnotations_whenMVCModelAttributeMethodExists_shouldGenerateInitializer(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenMVCModelAttributeMethodHasNoName_shouldReturnValidationError(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
