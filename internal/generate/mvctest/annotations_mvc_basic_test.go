...
func TestGenerateAnnotations_whenMVCRestControllerExists_shouldGenerateRestControllerConfiguration(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenMVCHeadAndOptionsRoutesExist_shouldGenerateMethodHelpers(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenMVCRequestParametersExist_shouldGenerateParameterBindings(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
