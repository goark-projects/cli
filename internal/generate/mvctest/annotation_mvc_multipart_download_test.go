...
func TestGenerateAnnotations_whenMVCMultipartBodyExists_shouldGenerateBindMultipartHandler(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenMVCMultipartBodySelectorMissing_shouldReturnValidationError(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
