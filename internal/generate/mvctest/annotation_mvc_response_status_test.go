...
func TestGenerateAnnotations_whenMVCResponseStatusOnNoReturn_shouldWrapNoContentHandler(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
`
	...
	if !strings.Contains(text, `mvc.DELETE("/jobs/{id}", mvc.ResponseStatus(202, mvc.NoContent`) {
		t.Fatalf(
			"generated mvc no-return handler should be wrapped with response status:\n%s",
			text,
		)
	}
}
...
`
	...
	if err == nil ||
		!strings.Contains(err.Error(), `must not declare both mapping status and response-status`) {
