...
func TestGenerateAnnotations_whenEmptyMVCCrossOriginExists_shouldGenerateDefaultCORSOption(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenMVCCrossOriginMethodHasNoRoute_shouldReturnValidationError(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
