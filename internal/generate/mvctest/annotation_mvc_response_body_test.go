...
func TestGenerateAnnotations_whenMVCRestControllerResponseBodyExists_shouldKeepExplicitBodyWrapper(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
`
	...
	if !strings.Contains(text, `mvc.GET("/api/status", mvc.ResponseBody[any](200`) {
		t.Fatalf(
			"generated rest controller response body source should keep explicit ResponseBody:\n%s",
			text,
		)
	}
}

func TestGenerateAnnotations_whenMVCResponseBodyUsesModel_shouldReturnValidationError(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
`
	...
	if err == nil ||
		!strings.Contains(err.Error(), `response-body must not be used with *mvc.Model`) {
