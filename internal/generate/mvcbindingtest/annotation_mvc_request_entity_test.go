...
func TestGenerateAnnotations_whenMVCRequestEntityAnnotated_shouldGenerateBindRequestEntity(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
func TestGenerateAnnotations_whenMVCRequestEntityValidatedExists_shouldGenerateGroups(
	t *testing.T,
) {
	dir := t.TempDir()
	source := `package app
...
`
	...
	for _, fragment := range expected {
		if !strings.Contains(text, fragment) {
			t.Fatalf(
				"generated request entity validation groups source missing %q:\n%s",
				fragment,
				text,
			)
		}
...
`
	...
	if err == nil ||
		!strings.Contains(
			err.Error(),
			"request entity parameter input must be goark.dev/goark/web.RequestEntity[T]",
		) {
