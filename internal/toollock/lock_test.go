	...
	lock := File{
		...
		Tools: []Entry{
			...
			},
			{
				...
				GOOS:          "linux",
				GOARCH:        "amd64",
				Package:       "example.com/tools/alpha",
				Version:       "v1.2.3",
				Module:        "example.com/tools",
				ModuleVersion: "v1.2.3",
				ModuleSum:     "h1:sum",
				Path:          "go/" + strings.Repeat("d", 64) + "/bin/alpha",
...
func TestRead_whenLockIsInvalid_shouldReject(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid digest", content: "version = 1\nbuild-sha256 = 'short'\n", want: "SHA-256"},
		{
			...
			content: lockWithTool("unsupported", "", "", "", "", "", "/tmp/demo"),
			want:    "类型",
		},
		{
			name: "Go tool missing module sum",
			content: lockWithTool(
				"go",
				"example.com/tools/demo",
				"v1.0.0",
				"example.com/tools",
				"v1.0.0",
				"",
				"go/"+strings.Repeat("c", 64)+"/bin/demo",
			),
			want: "module-sum",
		},
		{
			name: "system tool has Go metadata",
			content: lockWithTool(
				"system",
				"example.com/tools/demo",
				"v1.0.0",
		...
		{
			name: "Go tool uses absolute path",
			content: lockWithTool(
				"go",
				"example.com/tools/demo",
				"v1.0.0",
		...
		{
			name:    "local tool path escapes",
			content: lockWithTool("local", "", "", "", "", "", "../demo"),
			want:    "项目内",
		},
		{
			name:    "system tool uses relative path",
			content: lockWithTool("system", "", "", "", "", "", "demo"),
			want:    "绝对路径",
		},
		{
			name:    "system tool path is not canonical",
			content: lockWithTool("system", "", "", "", "", "", "/usr//bin/demo"),
			want:    "绝对路径",
		},
	}
...
func TestFileVerifyBuild_whenDescriptionChanged_shouldReject(t *testing.T) {
	...
	if err := lock.VerifyBuild(strings.Repeat("b", 64)); err == nil ||
